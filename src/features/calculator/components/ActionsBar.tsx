...
export default function ActionsBar({ onDelete, onClear, alignSide = 'start' }: Props) {
  ...
  const DeleteBtn = (
    <Pressable onPress={onDelete} style={[styles.actionBtn, { backgroundColor: '#BDBDBD', borderColor: '#BDBDBD' }]}>
  ...
  const ClearBtn = (
    <Pressable onPress={onClear} style={[styles.actionBtn, { backgroundColor: '#D32F2F', borderColor: '#D32F2F' }]}>
      <View style={[styles.inner, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
...
const styles = StyleSheet.create({
  ...
  actionBtn: { minWidth: 90, height: 40, borderRadius: 10, borderWidth: 1.5, alignItems: 'center', justifyContent: 'center' },
