...
type Props = TextProps & {
  ...
  maxWidth?: number;          // רוחב פנוי של הסלוט (פיקסלים)
  horizontalPadding?: number; // padding אופקי בתוך הסלוט
};
...
export default function AutoShrinkText({
  ...
  const onTextLayout = (e: NativeSyntheticEvent<TextLayoutEventData>) => {
    ...
    if (safetyRef.current++ > 40) { doneRef.current = true; return; }
    ...
    if (limitWidth != null) {
      for (const ln of lines) {
        if ((ln?.width ?? 0) > limitWidth + 0.1) { fitsWidth = false; break; }
      }
    }

    // מניעת שבירת מילה לשתי שורות: בודקים גבולות בין שורות
    let wordSplit = false;
    for (let i = 0; i < lines.length - 1; i++) {
      ...
      // אם הגבול אינו רווח/מקף/נקודה/פסיק וכו' → כנראה שבר מילה
